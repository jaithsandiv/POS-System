...
{
    public partial class UC_Category_Management : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void InitializeRepositoryItems()
        {
            ...
            gridCategories.RepositoryItems.Add(repositoryItemButtonEdit_Edit);
            gridCategories.RepositoryItems.Add(repositoryItemButtonEdit_Delete);
        }
        ...
        private void ConfigureGrid()
        {
            ...
            var colId = gridView1.Columns.AddVisible("category_id", "ID");
            colId.OptionsColumn.AllowEdit = false;
            colId.Width = 50;

            var colName = gridView1.Columns.AddVisible("category_name", "Name");
        ...
        private void LoadData()
        {
            try
            {
                DataTable dt = _bllProducts.GetCategories();
                gridCategories.DataSource = dt;
            }
            catch (Exception ex)
            {
        ...
        {
            Main.Instance.LoadUserControl(new UC_Category_Registration());
        }
        ...
        private void RepositoryItemButtonEdit_Edit_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
             ...
             if (row != null)
             {
                 ...
                 Main.Instance.LoadUserControl(new UC_Category_Registration(id));
             }
        }
        ...
        private void RepositoryItemButtonEdit_Delete_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            if (row != null)
            {
                if (XtraMessageBox.Show("Are you sure you want to delete this category?", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    int id = Convert.ToInt32(row["category_id"]);
                    int userId = 1; // Default
                    if (_bllProducts.DeleteCategory(id, userId))
                    {
                        LoadData();
                    }
                }
